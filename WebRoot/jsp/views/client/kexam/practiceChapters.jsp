...
<meta http-equiv="description" content="体验模考主页_章节练习" />
	...
	<div>
		<div class="mk_tims_dbuttom_cont">
			<div class="mk_tims_nr">
				<span class="mk_zsyd_title">
					${kind.nmProduct } <br /> 
					<label style="color:#939393; font-size:15px;">章节练习</label> 
				</span>
				
				<c:choose>
				<c:when test="${isBuy }">
				<c:forEach items="${pageEnt.listPages }" var="item" varStatus="itemStatus">
					<c:set value="${isBuy || itemStatus.index == 0 }" var="isCanSet"></c:set>
					<c:if test="${isCanSet }">
						<div class="mk_zjlx_cont"
							onmouseout="this.className='mk_zjlx_cont'"
							onmouseover="this.className='mk_zjlx_cont_hover'"
			...
			<c:when test="${isBuy }">
			<!-- 分页 -->
			<p:pageTag name="pageEnt" action="client/practiceChapters" />
			</c:when>
			</c:choose>
		</div>
	</div>
