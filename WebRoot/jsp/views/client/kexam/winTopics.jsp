...
<meta http-equiv="description" content="体验模考主页_绝胜押题" />
	...
	<div>
		<div class="mk_tims_dbuttom_cont">
			<div class="mk_tims_nr">
				<span class="mk_zsyd_title" style="border-bottom:0px;">${kind.nmProduct }<br /> <label style="color:#939393; font-size:15px;">绝胜押题</label>
				</span>
				
				<c:choose>
				<c:when test="${isBuy }">
					<c:forEach items="${pageEnt.listPages }" var="item">
						<div class="mk_jsyt_cont">
	            			<div class="mk_jsyt_style">
								<span class="mk_jsyt_title">${item.name }（${item.score }分）</span>
								...
								<span class="mk_jsyt_list_div">总题量：${numQues}</span> 
			                    <span class="mk_jsyt_list_div">最高分：${item.score }</span>
			                    <!-- 
			                    <span class="mk_jsyt_list_div">我的分数：<label style="color:#0060ff;">未做</label></span>
			                    <span class="mk_jsyt_list_div">正确率：<label style="color:#0060ff;">未做</label></span>
			                     -->
			                ...
			                <div class="mk_jsyt_bottom" onmouseout="this.className='mk_jsyt_bottom'" onmouseover="this.className='mk_jsyt_bottom_hover'" onclick="intoExamRoom(${item.id})">进入考场</div>
						</div>
					</c:forEach>
			...
			<c:when test="${isBuy }">
			<!--分页-->
			<p:pageTag name="pageEnt" action="client/winTopics"/>
			</c:when>
			</c:choose>
		</div>
	</div>
